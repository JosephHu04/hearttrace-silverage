...
实时文本陪伴默认使用阿里云百炼 `qwen3.8-flash`，异步关怀分析继续使用 `qwen-plus`；危机判断和紧急回复由服务端确定性规则处理，不依赖模型。密钥仅从本机环境变量读取。接入方式和合成案例评测说明见 [Qwen 接入与评测](docs/qwen-integration-and-evaluation.md)。
...
- 核心 API：演示 JWT、服务端 RBAC、家属关系授权与即时撤销、设备绑定隔离、风险复核、紧急求助、幂等与乐观版本、审计日志、SQLite/PostgreSQL 配置及 Alembic 迁移。
