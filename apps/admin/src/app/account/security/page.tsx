...
export default function SecurityPage() {
  ...
  useEffect(() => {
    try {
      const session = JSON.parse(sessionStorage.getItem("hearttrace.admin.session") ?? "null");
      if (!session?.accessToken || session.actor?.role !== "admin" || !Number.isFinite(Date.parse(session.expiresAt)) || Date.parse(session.expiresAt) <= Date.now()) throw new Error("expired");
      setToken(session.accessToken);
    } catch {
      sessionStorage.removeItem("hearttrace.admin.session");
  ...
  if (!token) return <main className="admin-auth-page"><p role="status">正在确认您的账号…</p></main>;
  return <main className="admin-auth-page"><section className="admin-auth-card security-card">
    <p className="admin-auth-eyebrow">心迹银龄 · 账号安全</p>
